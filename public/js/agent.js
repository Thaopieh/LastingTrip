$(document).ready(function () {
        ...
        data.forEach(function (hotel, index) {
          ...
          tableHtml +=
            ...
            tableHtml +=
                        ...
                        hotel.id + '">Dịch Vụ</button>';

          // Thêm icon với thuộc tính data-id chứa ID của khách sạn
          tableHtml +=
            '<td><i class="fa-solid fa-image" data-id="' + hotel.id + '"></i></td>';
  ...
  $(document).on("click", ".ManageService", function () {
    ...
    window.location.href = `/agent/hotel/Service`;
});
  ...
  $(document).on("click", ".addRoom", function () {
    let id = $(this).val();
    localStorage.setItem("hotelId", id);
    window.location.href = `/agent/room/`;
  });
  ...
  $(document).on("click", ".delete-image-btn", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        renderPage();
        $.ajax({
          url: "http://localhost:3030/api/v1/urlImageHotel/?HotelId=" + hotelId1,
          method: "GET",
          success: function (data) {
            console.log(data);
            // Xử lý khi nhận được danh sách các ảnh từ server
