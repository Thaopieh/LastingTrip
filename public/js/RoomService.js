$(document).ready(function () {
    var room = localStorage.getItem("IdRoom");
      function renderPage() {
          $.ajax({
              ...
              success: function (data) {
                ...
                tableHtml += "<td>";
                      tableHtml +=
                          ...
                          service.id + '">Chỉnh sửa</button>';
                      tableHtml +=
                          '<button type="button" class="deleteRoom" value="' +
                          service.id + '">Xóa</button>';
                      tableHtml += "</td>";
                      tableHtml += "</tr>";
                  });
      ...
      async function ServiceRoom() {
        try {
            ...
            const response = await $.ajax({
                url: 'http://localhost:3030/api/v1/amenities',
                data: { type: 1 }, // Assuming you want to fetch amenities of type 0
                method: 'GET'
            });
      ...
      $(document).on("click", ".updateService", function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (Array.isArray(options)) {
                    const matchedOption = options.find(option => option.id === data[0].serviceId);
                ...
                $("#serviceDropdown").change(function() {
                    ...
                    const selectedOption = options.find(option => option.id === parseInt(selectedOptionId));
                    if (selectedOption) {
